	...
	<div class="container-fluid">
		...
		<form action="findwines" method="post">
			...
			<div class="form-group pt-5 mt-5 row">
				...
				<input id="winename" class="form-control w-25 mr-4" 
					name="winename" value="${fn:escapeXml(param.winename)}"/>
					<button type="submit" class="btn btn-primary">Search</button>
					<button type="submit" class="btn btn-danger ml-1">Create Wine</button>
			</div>
		...
		<br /> <span id="successMessage"><h1><b>${messages.success}</b></h1></span>
		...
		<table border="1">
			...
			<c:forEach items="${wines}" var="wine">
				<tr>
					...
					<td><a href="winery?id=<c:out value="${wine.getWineryName()}"/>"><c:out value="${wine.getWineryName()}" /></a></td>
				</tr>
			</c:forEach>
		</table>
	</div>
